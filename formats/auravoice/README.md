# AURAVOICE Format Specification

**Format:** `.auravoice`
**Status:** Draft v1.0

This document defines the official **binary voicebank format** used by **Auraloid**.

`.auravoice` files are single-file containers that bundle metadata, phoneme definitions, audio data, and optional neural models for singing synthesis.
...
Used by editors and UI only. The engine must not rely on this block for synthesis logic.
...
Internal structure is **engine-defined and opaque**.
...
The internal structure is opaque to users and editors.
